	...
	public function index() {
		...
		if ($product_info) {
			...
			foreach ($results as $result) {
				$data['images'][] = array(
					'popup1' => $this->model_tool_image->resize($result['image'], $this->config->get('theme_' . $this->config->get('config_theme') . '_image_popup_width'), $this->config->get('theme_' . $this->config->get('config_theme') . '_image_popup_height')),
					'popup' => $this->model_tool_image->original($result['image']),
			...
			if ($product_info['image']) {
				$data['image'] = $this->model_tool_image->resize($product_info['image'], $this->config->get('theme_' . $this->config->get('config_theme') . '_image_related_width'), $this->config->get('theme_' . $this->config->get('config_theme') . '_image_related_height'));
			} else if(isset($data['images'][0])) {
				...
				foreach ($option['product_option_value'] as $option_value) {
					if (!$option_value['subtract'] || true/* || ($option_value['quantity'] > 0)*/) {
						...
						$product_option_value_data[] = array(
							...
							'image_f'				  => $this->model_tool_image->resize($option_value['image'], 1024, 1024),
							'price'                   => $price,
